# Ambiente DEV: compõe os módulos (rede, mensageria, computação, base de dados)
# e publica a configuração de runtime no SSM Parameter Store.

locals {
  # Prefixo de nomes, ex.: "microservices-dev".
...
# A aplicação lê estes parâmetros em vez de ter credenciais no código.
